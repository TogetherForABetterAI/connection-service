...
-- Create composite index for common query pattern (user_id + session_status)
CREATE INDEX IF NOT EXISTS idx_client_sessions_client_status ON client_sessions(user_id, session_status);
...
COMMENT ON COLUMN client_sessions.user_id IS 'Identifier for the client associated with this session';
